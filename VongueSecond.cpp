...
int penalty(int run_time) {
    if (run_time <= 180) return 0;
    run_time -= 180;

    int ret;
    if (run_time % 5 == 0) ret = ceil(run_time / 5) + 1;
    else ret = ceil(run_time / 5);
...
void graph_traversal(int t, int v, vector <int> visited_v, vector <int> dots_cnt) {
    if ((v == 0) and (t <= 215)) {                              //215 так как если мы опоздаем на 35 минут,
        int point = counter(visited_v) + penalty(t);     //то из-за штрафа мы не наберем больше, чем
        if (point > maxp) {                                                 //наш лучший полученный маршрут (16 баллов)
            maxp = point;
            max_route = visited_v;
            //cout << point << eol;
        }
        if (point == 16) print(visited_v);        // Доп. вывод всех максимальных маршрутов после того,
        // как было получено, что максмум 16 баллов
    }

    if (t >= 220) return;
    else {
        for (auto c: g[v]) {
            if (dots_cnt[c] > 10) return;       //См. файл Приложение
            dots_cnt[c]++;
